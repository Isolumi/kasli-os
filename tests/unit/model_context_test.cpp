...
TEST_CASE("ask with tool sends tool evidence to model and audits model request") {
  ...
  auto answer = service.ask_with_tool("Why did ssh fail?",
  ...
  REQUIRE(answer == "Fake answer based on 1 evidence record(s).");
  REQUIRE(provider.calls == 1);
...
TEST_CASE("ask with denied tool skips model request") {
  ...
  REQUIRE(answer == "Unable to answer because tool call did not succeed: "
...
TEST_CASE("ask with tool error skips model request") {
  ...
  REQUIRE(answer == "Unable to answer because tool call did not succeed: tool exploded");
  REQUIRE(provider.calls == 0);
